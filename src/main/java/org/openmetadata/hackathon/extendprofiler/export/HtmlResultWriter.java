...
public class HtmlResultWriter {
    ...
    public void write(List<ProfileResult> results) throws IOException {
        ...
        try (Writer w = new FileWriter(outputPath)) {
            ...
            w.write("<div class=\"selector-row\">\n");
            w.write("  <label>Select Table</label>\n");
            w.write("  <select id=\"table-selector\" onchange=\"showTable()\">\n");
            for (int i = 0; i < results.size(); i++) {
                ProfileResult pr = results.get(i);
                double ts = computeTableScore(pr);
                String icon = ts >= 80 ? "✅" : ts >= 50 ? "⚠️" : "❌";
                w.write("    <option value=\"" + i + "\">" + icon + " " + esc(pr.getTableFqn())
                        + "  (score: " + (int) ts + ")</option>\n");
            }
            w.write("  </select>\n");
            w.write("</div>\n");

            // --- per-table cards (hidden by default, JS shows selected) ---
            for (int i = 0; i < results.size(); i++) {
                writeTableCard(w, results.get(i), i);
            }
            ...
            w.write("<script>\n");
            w.write("function showTable() {\n");
            w.write("  document.querySelectorAll('.table-card').forEach(function(el) { el.style.display = 'none'; });\n");
            w.write("  var idx = document.getElementById('table-selector').value;\n");
            w.write("  var card = document.getElementById('tbl-' + idx);\n");
            w.write("  if (card) card.style.display = 'block';\n");
            w.write("}\n");
            w.write("</script>\n");
            w.write("</body>\n</html>\n");
    ...
    private void writeTableCard(Writer w, ProfileResult r, int index) throws IOException {
        ...
        String display = index == 0 ? "" : " style=\"display:none\"";
        w.write("<div class=\"card table-card\" id=\"tbl-" + index + "\"" + display + ">\n");
        ...
        if (!r.getColumnMetrics().isEmpty()) {
            List<String> metricNames = r.allColumnMetricNames();
    ...
    private static final String CSS = ""
            ...
            + ".selector-row label { font-size: 0.85em; font-weight: 600; color: #636e72; white-space: nowrap; }\n"
            + ".selector-row select { flex: 1; max-width: 600px; padding: 10px 14px; border-radius: 8px;\n"
            + "  border: 1px solid #dfe6e9; font-family: monospace; font-size: 0.9em; background: #fff; cursor: pointer; }\n"
            + ".selector-row select:focus { outline: 2px solid #6c5ce7; outline-offset: 2px; }\n"
